...
@section('content')
<h1 class="text-2xl font-semibold mb-6">Modifier chambre {{ $chambre->numero }}</h1>

<form action="{{ route('admin.chambres.update', $chambre) }}" method="POST" class="max-w-xl space-y-4">
    @csrf
    @method('PUT')
    <div>
    ...
    <div>
        ...
        <input type="text" name="numero" id="numero" required value="{{ old('numero', $chambre->numero) }}"
