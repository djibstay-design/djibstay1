...
@section('content')
<h1 class="text-2xl font-semibold mb-6">Modifier {{ $typeChambre->nom_type }}</h1>

<form action="{{ route('admin.types-chambre.update', $typeChambre) }}" method="POST" class="max-w-xl space-y-4">
    @csrf
    @method('PUT')
    <div>
        ...
        <select name="hotel_id" id="hotel_id" required class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded">
            @foreach ($hotels as $h)
                <option value="{{ $h->id }}" {{ old('hotel_id', $typeChambre->hotel_id) == $h->id ? 'selected' : '' }}>{{ $h->nom }}</option>
            @endforeach
        </select>
    </div>
    <div>
        <label for="nom_type" class="block text-sm font-medium mb-1">Nom du type *</label>
        <input type="text" name="nom_type" id="nom_type" required value="{{ old('nom_type', $typeChambre->nom_type) }}"
    ...
    <div>
        ...
        <input type="number" name="capacite" id="capacite" required min="1" value="{{ old('capacite', $typeChambre->capacite) }}"
            class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded">
    </div>
    <div>
